...
/**
 * This object exposes package private utilities for other classes. Thus, all methods are assumed to be invoked with
 * correct arguments, so these are not checked at all.
 */
internal object MathUtils {
    ...
    private const val Q_10: Int = 41

    private const val C_10: Long = 661971961083L

    private const val A_10: Long = -274743187321L

    private const val Q_2 = 38

    private const val C_2 = 913_124_641_741L

    private val POW_10 = longArrayOf(
            1L,
            10L,
            100L,
            1_000L,
    ...
    /**
     * The precomputed values for [g1] and [g0]. The first entry must be for an exponent of [K_MIN] or less. The last
     * entry must be for an exponent of [K_MAX] or more.
     */
     ...
     *
     * @param e The exponent which must meet 0 &le; `e` &le; [H].
     *
     * @return 10<sup>`e`</sup>.
     */
    internal fun pow10(e: Int): Long {
        return POW_10[e]
    }

    /**
     * Returns the unique integer __k__ such that 10<sup>__k__</sup> &le; 2<sup>`e`</sup> &lt; 10<sup>__k__+1</sup>.
     *
     * The result is correct when |`e`| &le; 5_456_721. Otherwise, the result is undefined.
     *
     * @param e The exponent of 2, which should meet |`e`| &le; 5_456_721 for safe results.
     *
     * @return &lfloor;log<sub>10</sub>2<sup>`e`</sup>&rfloor;.
     */
    internal fun flog10pow2(e: Int): Int {
        return (e * C_10 shr Q_10).toInt()
    }

    /**
     * Returns the unique integer __k__ such that
     * 10<sup>__k__</sup> &le; 3/4 &middot; 2<sup>`e`</sup> &lt; 10<sup>__k__+1</sup>.
     *
     * The result is correct when -2_956_395 &le; `e` &le; 2_500_325. Otherwise, the result is undefined.
     *
     * @param e The exponent of 2, which should meet -2_956_395 &le; `e` &le; 2_500_325 for safe results.
     *
     * @return &lfloor;log<sub>10</sub>(3/4 &middot; 2<sup>`e`</sup>)&rfloor;.
     */
    internal fun flog10threeQuartersPow2(e: Int): Int {
        return (e * C_10 + A_10 shr Q_10).toInt()
    }

    /**
     * Returns the unique integer __k__ such that 2<sup>__k__</sup> &le; 10<sup>`e`</sup> &lt; 2<sup>__k__+1</sup>.
     *
     * The result is correct when |`e`| &le; 1_838_394. Otherwise, the result is undefined.
     *
     * @param e The exponent of 10, which should meet |`e`| &le; 1_838_394 for safe results.
     *
     * @return &lfloor;log<sub>2</sub>10<sup>`e`</sup>&rfloor;.
     */
    internal fun flog2pow10(e: Int): Int {
        return (e * C_2 shr Q_2).toInt()
    }

    /**
     * Let 10<sup>-`k`</sup> = __&beta;__ 2<sup>__r__</sup>, for the unique pair of integer __r__ and real __&beta;__
     * meeting 2<sup>125</sup> &le; __&beta;__ &lt; 2<sup>126</sup>. Further, let
     * __g__ = &lfloor;__&beta;__&rfloor; + 1. Split __g__ into the higher 63 bits __g__<sub>1</sub> and the lower 63
     * bits __g__<sub>0</sub>. Thus, __g__<sub>1</sub> = &lfloor;__g__ 2<sup>-63</sup>&rfloor; and
     * __g__<sub>0</sub> = __g__ - __g__<sub>1</sub> 2<sup>63</sup>.
     *
     * This method returns __g__<sub>1</sub> while [g0] returns __g__<sub>0</sub>.
     *
     * If needed, the exponent __r__ can be computed as __r__ = `flog2pow10(-k)` - 125 (see [flog2pow10]).
     *
     * @param k The exponent of 10, which must meet [K_MIN] &le; `e` &le; [K_MAX].
     *
     * @return __g__<sub>1</sub> as described above.
     */
    internal fun g1(k: Int): Long {
        return G[k - K_MIN shl 1]
    }

    /**
     * Returns __g__<sub>0</sub> as described in [g1].
     *
     * @param k The exponent of 10, which must meet [K_MIN] &le; `e` &le; [K_MAX].
     *
     * @return __g__<sub>0</sub> as described in [g1].
     */
    internal fun g0(k: Int): Long {
        return G[k - K_MIN shl 1 or 1]
    }

    internal fun multiplyHigh(x: Long, y: Long): Long {
        ...
        val y1 = y ushr 32
        val a = x1 * y1
        return ((b ushr 32) + (x1 + x2) * (y1 + y2) - b - a ushr 32) + a
    }
