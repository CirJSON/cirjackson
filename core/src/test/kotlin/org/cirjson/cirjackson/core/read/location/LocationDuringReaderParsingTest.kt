...
class LocationDuringReaderParsingTest : TestBase() {

    @Test
    fun testLocationAtEndOfParse() {
        for (locationTestCase in LocationTestCase.entries) {
            locationAtEndOfParse(MODE_READER, locationTestCase)
            locationAtEndOfParse(MODE_READER_THROTTLED, locationTestCase)
        }
    }
    ...
    fun testInitialLocation() {
        for (locationTestCase in LocationTestCase.entries) {
            initialLocation(MODE_READER, locationTestCase)
            initialLocation(MODE_READER_THROTTLED, locationTestCase)
        }
    }
    ...
    fun testTokenLocation() {
        for (locationTestCase in LocationTestCase.entries) {
            tokenLocation(MODE_READER, locationTestCase)
            tokenLocation(MODE_READER_THROTTLED, locationTestCase)
        }
    }
